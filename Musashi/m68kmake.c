	...
	if(cpu_type < CPU_TYPE_020)
	{
		if(cpu_type == CPU_TYPE_010)
		{
			if(strcmp(op->name, "moves") == 0)
				return (int)op->cycles[cpu_type] + g_moves_cycle_table[ea_mode][size];
			if(strcmp(op->name, "clr") == 0)
				return (int)op->cycles[cpu_type] + g_clr_cycle_table[ea_mode][size];
		}
		...
		if(cpu_type == CPU_TYPE_000 && ea_mode == EA_MODE_I &&
		   ((strcmp(op->name, "add") == 0 && strcmp(op->spec_proc, "er") == 0) ||
			...
			return (int)op->cycles[cpu_type] + g_ea_cycle_table[ea_mode][cpu_type][size] + 2;

		if(strcmp(op->name, "jmp") == 0)
			return (int)op->cycles[cpu_type] + g_jmp_cycle_table[ea_mode];
		if(strcmp(op->name, "jsr") == 0)
			return (int)op->cycles[cpu_type] + g_jsr_cycle_table[ea_mode];
		if(strcmp(op->name, "lea") == 0)
			return (int)op->cycles[cpu_type] + g_lea_cycle_table[ea_mode];
		if(strcmp(op->name, "pea") == 0)
			return (int)op->cycles[cpu_type] + g_pea_cycle_table[ea_mode];
	}
	return (int)op->cycles[cpu_type] + g_ea_cycle_table[ea_mode][cpu_type][size];
}
...
void add_opcode_output_table_entry(opcode_struct* op, char* name)
{
	...
	*ptr = *op;
	...
	ptr->bits = (unsigned int)num_bits((int)ptr->op_mask);
}
...
static int DECL_SPEC compare_nof_true_bits(const void* aptr, const void* bptr)
{
	...
	if(a->op_mask != b->op_mask)
		return (int)(a->op_mask - b->op_mask);
	return (int)(a->op_match - b->op_match);
}

void print_opcode_output_table(FILE* filep)
{
	int i;
	qsort((void *)g_opcode_output_table, (size_t)g_opcode_output_table_length, sizeof(g_opcode_output_table[0]), compare_nof_true_bits);
...
void set_opcode_struct(opcode_struct* src, opcode_struct* dst, int ea_mode)
{
	...
	*dst = *src;
	...
	for(i=0;i<NUM_CPUS;i++)
		dst->cycles[i] = (unsigned int)get_oper_cycles(dst, ea_mode, i);
...
void generate_opcode_cc_variants(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* op_in, int offset)
{
	...
	*op = *op_in;
	...
	for(i=2;i<16;i++)
	{
		...
		op->op_match = (op->op_match & 0xf0ff) | ((unsigned int)(i<<8));
...
void populate_table(void)
{
	...
	for(op = g_opcode_input_table;;op++)
	{
		...
		for(i=0;i<16;i++)
		{
			op->op_mask |= (unsigned int)((bitpattern[i] != '.') << (15-i));
			op->op_match |= (unsigned int)((bitpattern[i] == '1') << (15-i));
		}
	}
	/* Terminate the list */
	op->name[0] = 0;
